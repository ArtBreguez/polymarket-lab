...
class TestAsyncGammaClient:
    ...
    async def test_fetch_markets(self):
        respx.get("https://mock-gamma.test/markets").mock(return_value=httpx.Response(200, json=[{"question": "Will it rain?"}]))
    ...
    async def test_keyword_filter(self):
        respx.get("https://mock-gamma.test/markets").mock(return_value=httpx.Response(200, json=[{"question": "Will it rain in London?"}, {"question": "Will it snow in Paris?"}]))
        async with AsyncGammaClient(base_url="https://mock-gamma.test") as client:
...
class TestAsyncClobClient:
    ...
    async def test_fetch_prices(self):
        respx.get("https://mock-clob.test/midpoint").mock(return_value=httpx.Response(200, json={"mid": "0.65"}))
