...
def status_cmd(
    ...
    if champion_path.exists():
        ...
        segs = [
            f"[green]{r['segment']}[/green]" if r["passes"]
            else f"[red]{r['segment']}[/red]"
...
def scan_markets_cmd(
    ...
    tag: Optional[str] = typer.Option(None, "--tag", help="Gamma API tag filter"),
    keyword: Optional[str] = typer.Option(None, "--keyword", help="Keyword filter"),
    ...
    rprint(f"[bold]Scanning markets[/bold] (plugin={plugin}, tag={tag or 'none'}, keyword={keyword or 'none'})")
...
def record_trades_cmd(
    ...
    artifacts_dir: str = typer.Option("artifacts", "--artifacts-dir", "-a", envvar="PMLAB_ARTIFACTS_DIR"),
    ...
    signals_path = Path(artifacts_dir) / "workspaces" / workspace / "signals" / "v2" / "scan_edge_signals.json"
    ...
    signals = [
        EdgeSignal(**s) for s in all_signals
        if float(s.get("best_edge", 0)) >= min_edge
    ]
    rprint(f"Found [cyan]{len(signals)}[/cyan] signals with edge >= {min_edge}")

    trades_path = Path(artifacts_dir) / "workspaces" / workspace / "signals" / "v2" / "forward_paper_trades.json"
    broker = PaperBroker(
    ...
    for t in new_trades:
        rprint(f"  + {t['city_or_segment']} {t['target_date']} {t['outcome_label']} @ {t['gamma_price']:.3f} (edge {t['edge_after_fee']:.3f})")
...
def settle_trades_cmd(
    ...
    rprint('[dim]Example: scripts/pmlab-workspace ops_daily python -c "from pmlab import SettlementEngine; ..."[/dim]')
...
def backtest_cmd(
    ...
    flat_stake: float = typer.Option(1.0, "--flat-stake"),
    output: Optional[str] = typer.Option(None, "--output", "-o", help="Save metrics JSON to path"),
) -> None:
    ...
    rprint(f"Panel: {len(panel)} rows, {panel['decision_date'].nunique()} dates, {panel['market_id'].nunique()} markets")

    forecaster = LGBMForecaster()
    with console.status("Running walk-forward backtest..."):
        result = rolling_origin_eval(panel, forecaster, min_train_rows=min_train_rows, stride=stride, flat_stake=flat_stake)
...
def promote_champion_cmd(
    ...
    rprint(f"Gate decision: [{'green' if gate.decision == 'GO' else 'red'}]{gate.decision}[/{'green' if gate.decision == 'GO' else 'red'}]")
    ...
    if gate.decision != "GO":
        ...
        for r in gate.segment_results:
            ...
            rprint(f"  {status} {r.segment}: {r.num_trades} trades, ${r.total_pnl:+.2f} ({r.reason})")
        raise typer.Exit(1)
    ...
    try:
        ...
        rprint(f"[green]Champion published![/green]")
